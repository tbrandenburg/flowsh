...
(
local CONV_ID="$(get_var "CONV_ID" "initialize_memory_system")"
...
CONTEXT_FILE=\"/tmp/ai_memory/context/\\\$CONV_ID_context.txt\"
...
(
local CONV_ID="$(get_var "CONV_ID" "load_conversation_history")"
/bin/bash -c "set -euo pipefail
...
(
local USER_INPUT="$(get_var "USER_INPUT" "save_conversation_turn")"
local LLM_CONTENT="$(get_var "LLM_CONTENT" "save_conversation_turn")"
local CONV_ID="$(get_var "CONV_ID" "save_conversation_turn")"
local HISTORY_FILE="$(get_var "HISTORY_FILE" "save_conversation_turn")"
/bin/bash -c "set -euo pipefail
...
(
local CONV_ID="$(get_var "CONV_ID" "update_context_summary")"
local USER_INPUT="$(get_var "USER_INPUT" "update_context_summary")"
local LLM_CONTENT="$(get_var "LLM_CONTENT" "update_context_summary")"
local CONTEXT_FILE="$(get_var "CONTEXT_FILE" "update_context_summary")"
/bin/bash -c "set -euo pipefail
...
tail -n 50 \"\$CONTEXT_FILE\" > \"\$CONTEXT_FILE.tmp\"
