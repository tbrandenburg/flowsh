...
# AI Content to Telegram - Production Ready - Generated by flowsh
# Registry-based workflow execution script

echo "Starting workflow: AI Content to Telegram - Production Ready"
...
if [[ -n "${OPENAI_API_KEY:-}" ]]; then
    ...
    llm_response=$(curl -s -X POST "https://api.openai.com/v1/chat/completions" \
      ...
      -d '{"model": "gpt-4", "messages": [{"role": "user", "content": "Create an engaging riddle about space exploration. Include both question and answer. Make it fun and educational."}]}' \
...
if [[ -z "$llm_content" ]]; then
    ...
    llm_response=$(curl -s -X POST "https://api.llm7.io/v1/chat/completions" \
      -H "Content-Type: application/json" \
      -d '{"model": "default", "messages": [{"role": "user", "content": "Create an engaging riddle about space exploration. Include both question and answer. Make it fun and educational."}]}' \
      --connect-timeout 30 --max-time 60)
    if extract_llm_content "$llm_response" >/dev/null 2>&1; then
        llm_content=$(extract_llm_content "$llm_response")
...
if [[ -z "$llm_content" ]]; then
    llm_content="Mock LLM Response: This is a simulated response because all API calls failed. Original prompt was: Create an engaging riddle about space exploration. Include both question and answer. Make it fun and educational."
...
# Node: report_status
echo "# 🎉 AI Content to Telegram - COMPLETE!

## 📊 Status Summary
- **AI Generation**: ✅ Success
- **Telegram Delivery**: $(get_var "TELEGRAM_SUCCESS" "report_status")
- **Environment**: ✅ Validated

## 🎯 Generated Content
$(get_var "LLM_CONTENT" "report_status")
...
**Status**: $(get_var "TELEGRAM_SUCCESS" "report_status")
"
