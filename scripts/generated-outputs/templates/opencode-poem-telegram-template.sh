...
# OpenCode Poem to Telegram - Creative Automation - Generated by flowsh
# Registry-based workflow execution script

echo "Starting workflow: OpenCode Poem to Telegram - Creative Automation"
...
AGENT_GENERATE_POEM_OUTPUT=${AGENT_GENERATE_POEM_OUTPUT:-""}
AGENT_GENERATE_POEM_EXIT_CODE=${AGENT_GENERATE_POEM_EXIT_CODE:-""}
AGENT_GENERATE_POEM_SUCCESS=${AGENT_GENERATE_POEM_SUCCESS:-""}
FALLBACK_POEM=${FALLBACK_POEM:-""}
...
echo \"🎭 Poem Type: \$(get_var "POEM_TYPE" "validate_env")\"
echo \"🎨 Theme: \$(get_var "POEM_THEME" "validate_env")\"
"

# Node: generate_poem
# Node: generate_poem (OpenCode Poetry Generator)
execute_agent_generate_poem() {
    log_step "🤖 Executing agent: OpenCode Poetry Generator"
    ...
    export POEM_TYPE="${poem_type:-haiku}"
    export POEM_THEME="${poem_theme:-nature and technology harmony}"

    # Prepare agent prompt
    local prompt_content
    read -r -d '' prompt_content << AGENT_PROMPT_EOF || true
Create a ${poem_type:-haiku} poem about "${poem_theme:-nature and technology harmony}".
...
AGENT_PROMPT_EOF
    ...
    if command -v timeout >/dev/null 2>&1; then
        agent_output=$(timeout 45 opencode run "$prompt_content" 2>&1) || exit_code=$?
    else
    ...
    set_workflow_var "agent_generate_poem_output" "$agent_output"
    set_workflow_var "agent_generate_poem_exit_code" "$exit_code"
    set_workflow_var "agent_generate_poem_success" "false"

    # Handle execution result
    if [[ $exit_code -eq 0 ]]; then
        set_workflow_var "agent_generate_poem_success" "true"
        log_success "Agent execution completed successfully"
        echo "$agent_output"
    else
        case $exit_code in
            124)
                log_error "Agent execution timed out after 45s"
                ;;
            127)
...
# Node: prepare_fallback
set_var "FALLBACK_POEM" "🌟 **Digital Haiku** 🌟
...
# Node: select_poem_content
# Node: select_poem_content
FINAL_POEM=$(if [ -n "'"$(get_workflow_var "AGENT_OUTPUT__" "default")"'" ]; then echo "'"$(get_workflow_var "AGENT_OUTPUT" "default")"'"; else echo "'"$(get_workflow_var "FALLBACK_POEM__DEFAULT_FALLBACK_POEM" "default")"'"; fi)
set_var "FINAL_POEM" "$FINAL_POEM" "select_poem_content"

# Node: format_telegram_message
set_var "TELEGRAM_MESSAGE" "🎭 <b>Poem of the Day</b> 🎭

<b>Type:</b> <code>\${poem_type:-haiku}</code>
<b>Theme:</b> <code>\${poem_theme:-nature and technology harmony}</code>
...
# Node: send_poem_telegram

# Node: send_poem_telegram (Send Poem to Telegram)
execute_telegram_send_poem_telegram() {
    log_step "📱 Sending Telegram message: Send Poem to Telegram"

    local message="$(get_var "TELEGRAM_MESSAGE" "send_poem_telegram")"
...
# Node: completion_report
echo "# 🎭 OpenCode Poem to Telegram - COMPLETE!

## 📊 Generation Summary
- **Poem Type**: $(get_var "POEM_TYPE" "completion_report")
- **Theme**: $(get_var "POEM_THEME" "completion_report")  
- **Agent Status**: ${agent_output:+✅ OpenCode Generated}${agent_output:-⚡ Fallback Used}
- **Telegram Delivery**: $(get_var "TELEGRAM_SUCCESS" "completion_report")
- **Environment**: ✅ Validated

## 🎨 Generated Content
$(get_var "FINAL_POEM" "completion_report")
...
**Delivery Status**: $(get_var "TELEGRAM_SUCCESS" "completion_report")
**Generated**: $(date)
"
