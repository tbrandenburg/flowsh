...
# Version: 1.0.0
# Generated: 2026-01-02T15:34:42.025Z
...
# Agent Node: agent_task
execute_node_agent_task() {
    local input_data="$1"
    local correlation_id="$2"
    
    log_operation_start "agent_task" "agent_execution" "$correlation_id"
    ...
    local temp_file_prefix="agent_agent_task_$correlation_id"
    ...
    if [[ "$USE_MOCK_TOOLS" == "true" ]]; then
        ...
        log_debug "$correlation_id" "Using mock agent execution"
        local mock_output="Mock output from agent agent_task at $(date)"
        
        echo "$mock_output" > "$output_file"
        exit_code=0
        
        log_debug "$correlation_id" "Mock agent completed successfully"
    else
        
        log_debug "$correlation_id" "Executing agent: opencode"
        
        # Execute with timeout and capture output
        if timeout 300 "opencode"  > "$output_file" 2> "$error_file"; then
            exit_code=0
        else
            exit_code=$?
            ...
            log_error "$correlation_id" "Agent execution failed" "{\"exit_code\": $exit_code, \"error\": \"$error_output\"}"
        fi
    fi
    ...
    if [ $exit_code -ne 0 ]; then
        case $exit_code in
            1)
                log_error "$correlation_id" "Agent execution general error" "{\"node\": \"agent_task\"}"
                ;;
            124)
                log_error "$correlation_id" "Agent execution timeout" "{\"node\": \"agent_task\"}"
                ;;
            126)
                log_error "$correlation_id" "Agent execution command not executable" "{\"node\": \"agent_task\"}"
                ;;
            127)
                log_error "$correlation_id" "Agent execution command not found" "{\"node\": \"agent_task\"}"
                ;;
            *)
                log_error "$correlation_id" "Agent execution unknown error" "{\"node\": \"agent_task\", \"exit_code\": $exit_code}"
                ;;
        esac
        return $exit_code
    fi
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        log_success "$correlation_id" "Agent execution completed" "{\"output_length\": ${#output}}"
        
        # Store output for downstream nodes
        set_workflow_var "agent_output" "$output"
        set_workflow_var "agent_task_output" "$output"
...
execute_workflow() {
    ...
    # Workflow flow generated automatically
