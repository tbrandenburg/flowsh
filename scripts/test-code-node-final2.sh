...
# Node: list_files
(
TARGET_DIRECTORY="$(get_var "TARGET_DIRECTORY" "list_files")"
FILE_PATTERN="$(get_var "FILE_PATTERN" "list_files")"
find "$TARGET_DIRECTORY" -name "$FILE_PATTERN" -type f
)

# Node: count_lines
(
TARGET_DIRECTORY="$(get_var "TARGET_DIRECTORY" "count_lines")"
FILE_PATTERN="$(get_var "FILE_PATTERN" "count_lines")"
sh -c "find $TARGET_DIRECTORY -name '$FILE_PATTERN' -type f -exec wc -l {} + | tail -1 | awk '{print \$1}' || echo '0'"
)

# Node: get_file_sizes
(
TARGET_DIRECTORY="$(get_var "TARGET_DIRECTORY" "get_file_sizes")"
FILE_PATTERN="$(get_var "FILE_PATTERN" "get_file_sizes")"
sh -c "find $TARGET_DIRECTORY -name '$FILE_PATTERN' -type f -exec du -ch {} + | tail -1 | cut -f1 || echo '0B'"
)

# Node: create_summary
(
FILE_PATTERN="$(get_var "FILE_PATTERN" "create_summary")"
TARGET_DIRECTORY="$(get_var "TARGET_DIRECTORY" "create_summary")"
sh -c "echo \"File Analysis Summary\" > /tmp/flowsh-code-example/summary.txt && echo \"Pattern: $FILE_PATTERN\" >> /tmp/flowsh-code-example/summary.txt && echo \"Directory: $TARGET_DIRECTORY\" >> /tmp/flowsh-code-example/summary.txt && echo \"Generated: \$(date)\" >> /tmp/flowsh-code-example/summary.txt"
)
