# Dify Workflow YAML Schema Documentation

## Overview

This document provides comprehensive documentation for the Dify Workflow YAML schema, which has been reverse-engineered from the Dify codebase. The schema allows workflows to be defined as exportable YAML configurations with complete validation and type safety.
...
Dify implements a sophisticated multi-layered template system that enables reusability, sharing, and sophisticated prompt management across the entire platform. This system provides multiple approaches for template management depending on your use case.
...
template_libraries:
  ...
  - library_id: "community-hub"
    ...
    source_url: "https://templates.dify.ai"
    sync_config:
      auto_sync: true
      sync_interval: 86400
...
Dify implements nested workflows using a **"flat graph with references"** approach rather than true hierarchical nesting.
...
```yaml
# yaml-language-server: $schema=./dify-workflow-schema.yaml
```

Or configure your IDE to associate `.dify.yaml` files with the schema.
...
Workflows can be:
1. Exported from Dify UI to YAML format
2. Version controlled in Git repositories  
3. Imported back into Dify for execution
4. Shared between different Dify instances
