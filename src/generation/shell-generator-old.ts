...
function generateVariableSetup(nodes: WorkflowNode[]): string {
  ...
  for (const node of nodes) {
    if (node.type === 'variable-assignment' && 'variable' in node.data) {
      const sanitizedVar = sanitizeVariableName(String(node.data.variable));
      const varName = sanitizedVar.toUpperCase();
      const defaultValue = 'value' in node.data ? String(node.data.value) || '' : '';
      // Escape default value to prevent injection
      const escapedValue = defaultValue.replace(/"/g, '\\"');
      variables.push(`${varName}=\${${varName}:-"${escapedValue}"}`);
    }

    // Extract template variables from command strings
    if ('command' in node.data && node.data.command) {
      const matches = String(node.data.command).match(/\{\{(\w+)\}\}/g);
      if (matches) {
        for (const match of matches) {
          const rawVarName = match.replace(/\{\{|\}\}/g, '');
          const sanitizedVar = sanitizeVariableName(rawVarName);
          const varName = sanitizedVar.toUpperCase();
...
function generateCodeNode(node: WorkflowNode): string {
  ...
  // Replace template variables with proper sanitization
...
function generateVariableNode(node: WorkflowNode): string {
  const rawVariable =
    'variable' in node.data && node.data.variable ? String(node.data.variable) : 'TEMP_VAR';
  const variable = sanitizeVariableName(rawVariable);
  const rawValue = 'value' in node.data && node.data.value ? String(node.data.value) : '';
