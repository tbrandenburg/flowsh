...
export function generateShellScript(
  ...
  scriptParts.push(generateNodeFunctions(graph.nodes, options));
  
  // 6. Main execution flow
  scriptParts.push(generateMainFlow(graph.nodes, graph.edges || [], options));
...
function generateTemplateSystem(workflow: FlowshWorkflow): string {
  const dependencies = workflow.workflow?.template_dependencies || workflow.spec?.template_dependencies || [];
 ...
 */
function generateNodeFunctions(nodes: WorkflowNode[], _options: GenerationOptions): string {
 ...
 */
function generateMainFlow(nodes: WorkflowNode[], edges: WorkflowEdge[], _options: GenerationOptions): string {
  // Find start node
  ...
  if (startNodeId) {
    ...
    const remainingNodes = nodes.filter(node => node.id !== startNodeId && !isEndNode(node) && !isAnswerNode(node));
 ...
 */
function analyzeWorkflow(nodes: WorkflowNode[], edges: WorkflowEdge[]): GenerationResult['metadata'] {
